...
function getProviderStatus() { return getHealthReport(); }
...
async function sendMessage(messages, systemPrompt, options = {}) {
    ...
    try {
        ...
        if (error.status === 402 || error.status === 429) {
            try {
                const result = await callGroq(messages, systemPrompt, { ...options, maxTokens: Math.floor(maxTokens * 0.5) });
        ...
        try {
            const result = await callGroq(messages, systemPrompt, { ...options, model: GROQ_CONFIG.fallbackModel, maxTokens: Math.floor(maxTokens * 0.7) });
...
module.exports = { sendMessage, getHealthReport, getProviderStatus, getAnswerCacheKey, getCachedAnswer, setCachedAnswer, getAnswerCacheStats, buildKnowledgeFallback, GROQ_CONFIG };
